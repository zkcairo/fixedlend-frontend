...
const AllOffers = ({ offers, loading, type, me, labelButton, action, category }: Props) => {
  ...
  const renderOffer = (offer: any, index: number) => (
    <div key={index} className="flex flex-col gap-y-1 p-4 border border-green-500/30">
      ...
      <p>Available: {formatCurrency(Number(offer.amount_available), 10**7)} {categoryName(category)}</p>
      {me && <p>Total Size: {formatCurrency(Number(offer.total_amount), 10**7)} {categoryName(category)}</p>}
  ...
  const renderLoan = (loan: any, index: number, loanType: 'borrow' | 'lend') => {
      ...
      return (
          <div key={index} className="flex flex-col gap-y-1 p-4 border border-green-500/30">
              ...
              <p>Amount: {formatCurrency(Number(loan.amount), 10**7)} {categoryName(category)}</p>
              ...
              <p>Current Value: {formatCurrency(interest, 10**7)} {categoryName(category)}</p>
